...
# (파일명 패턴, 회사, 카테고리)
# (파일명, 회사, 카테고리, 고정 주제)
# 고정 주제가 있으면 그 파일의 모든 문항에 그 주제를 부여.
# 고정 주제가 None이면 아래 TOPIC_BY_NUM(번호별 주제)으로 개별 분류.
SOURCE_FILES = [
    ("6_18.md", "엑셀리언트", "실제 면접 기록", None),
    # 압토솔·올라핀테크는 예시 질문이라 제외. 다시 쓰려면 아래 주석 해제.
    # ("면접준비_프로젝트1_올라핀테크.md", "올라핀테크", "프로젝트 1", "프로젝트 1"),
    # ("면접준비_프로젝트2_올라핀테크.md", "올라핀테크", "프로젝트 2", "프로젝트 2"),
    # ("면접준비_프로젝트3_올라핀테크.md", "올라핀테크", "프로젝트 3", "프로젝트 3"),
]

# 엑셀리언트(6_18.md) 문항의 제목 앞 번호 -> 주제 카테고리
# 프로젝트 사례 / 일하는 방식 / 커뮤니케이션 / 나에 대한 질문
TOPIC_BY_NUM = {
    # 🧑 나에 대한 질문 (성격·장단점·학교·진로 등)
    "1": "나에 대한 질문", "2": "나에 대한 질문", "3": "나에 대한 질문",
    "3.1": "나에 대한 질문", "3.2": "나에 대한 질문", "4": "나에 대한 질문",
    "15": "나에 대한 질문", "16": "나에 대한 질문", "17": "나에 대한 질문",
    "18": "나에 대한 질문", "24": "나에 대한 질문", "25": "나에 대한 질문",
    "26": "나에 대한 질문", "27": "나에 대한 질문",
    # 🛠️ 기술·프로젝트 사례
    "5": "기술·프로젝트", "6": "기술·프로젝트", "6.1": "기술·프로젝트",
    "6.1.1": "기술·프로젝트", "6.2": "기술·프로젝트", "7": "기술·프로젝트",
    "8": "기술·프로젝트", "9": "기술·프로젝트", "11": "기술·프로젝트",
    "13.1": "기술·프로젝트", "19": "기술·프로젝트", "23": "기술·프로젝트",
    # ⚙️ 일하는 방식
    "12": "일하는 방식", "20": "일하는 방식", "21": "일하는 방식", "22": "일하는 방식",
    # 💬 커뮤니케이션
    "10": "커뮤니케이션", "28": "커뮤니케이션", "29": "커뮤니케이션", "30": "커뮤니케이션",
    # 📝 면접 마무리 (조건 협의)
    "31": "면접 마무리", "32": "면접 마무리", "33": "면접 마무리",
}


def topic_for(title: str, fixed_topic):
    """고정 주제가 있으면 그것을, 없으면 제목 번호로 주제를 정한다."""
    if fixed_topic:
        return fixed_topic
    # (추가 질문)은 직전 기술 문항의 후속이라 기술·프로젝트로
    if title.startswith("(추가 질문"):
        return "기술·프로젝트"
    m = re.match(r"(\d+(?:\.\d+)*)\.", title)
    if m:
        return TOPIC_BY_NUM.get(m.group(1), "기타")
    return "기타"
...
def main():
    all_cards = []
    qid = 0
    for fname, company, category, fixed_topic in SOURCE_FILES:
        ...
        for c in cards:
            ...
            c["company"] = company
            c["category"] = category
            c["topic"] = topic_for(c.get("title", ""), fixed_topic)
            c["source"] = fname
